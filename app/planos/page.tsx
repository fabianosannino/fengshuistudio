...
const PLANOS = [
  {
    ...
    recursos: [
      ...
      { nome: 'Geração de PDF', valor: 'Marca d\'água', disponivel: true },
  ...
  },
  {
    ...
    preco: 'R$ 49,90',
    periodo: '/mês',
    descricao: 'Para consultores profissionais',
    cor: '#7C3AED',
    destaque: true,
    recursos: [
...
export default function Planos() {
  ...
  async function handleSelectPlan(planoId: string) {
    ...
    if (planoId === 'pro') {
      // Por enquanto, simula a assinatura (futuramente integrar com gateway)
      const { error } = await supabase
        .from('profiles')
        .update({ plano: 'pro' })
        .eq('id', user.id)
      if (error) {
        setMessage('Erro ao atualizar plano: ' + error.message)
      } else {
    ...
    <div style={{ minHeight: '100vh', background: '#F9FAFB', fontFamily: 'Arial, sans-serif' }}>
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <span style={{ color: '#ffffff', fontSize: '14px', fontWeight: 'bold' }}>Planos</span>
