/**
 * Biblioteca canônica de Curas & Ativações — 9 setores × 6 modalidades
 * (cristais, plantas, objetos, mudra, meditação, mantras).
 *
 * Extraída de app/curas/page.tsx: conteúdo de domínio Feng Shui vive em
 * src/lib (regra do CLAUDE.md), não dentro de componente. Fonte única
 * para a tela de Curas e, futuramente, para o relatório e o motor dos
 * Cinco Elementos.
 */
...
export interface Crystal { nome: string; cor: string; propriedade: string }
export interface Plant { icon: string; nome: string; dica: string; posicao: string }
export interface Obj { icon: string; nome: string; posicao: string }
export interface Mudra { icon: string; nome: string; descricao: string; passos: string[] }
export interface Meditation { nome: string; duracao: string; descricao: string; passos: string[] }
export interface Mantra { caracteres: string; romanizacao: string; significado: string }
...
export const ELEMENTOS: ElementData[] = [
  // ── 1. ÁGUA / CARREIRA ──────────────────────────────────────────────────
  {
    id: 'agua-carreira',
    nome: 'Água',
