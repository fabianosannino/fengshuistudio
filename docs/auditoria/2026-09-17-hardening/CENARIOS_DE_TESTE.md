...
| E-POL-03 | Planta antiga com marcas → Editar, bordas, OK, nova falta/excesso, OK, concluir, recalcular → confirmar etapas sem perder referências/polígonos | Página real React/jsdom, mouse/touch/pen, sobreposição, troca de ferramenta com rascunho, seleção dos pontos, salvar/reabrir. Regressão adicional: sobreposição com dimensão positiva ao restaurar, alvos de 44 px CSS e resultado dos setores visível após OK; o teste reproduziu tamanho inicial zero antes do ajuste |
...
| D-MOB-03 | Duas identidades e escritas concorrentes em banco descartável → RPC/GET/excluir → uma revisão vence, alheio/anon negados, geometria concorrente rejeitada, consulta excluída não deixa cadastro órfão | `scripts/security/test-authorization.mjs`, PostgreSQL 17/PostgREST 16.1 no gate obrigatório do CI; grants e invoker conferidos em SQL. Aguardar a RPC no cache antes de testar autorização: coluna disponível não prova função disponível. Execução final vinculada ao PR #217 |
