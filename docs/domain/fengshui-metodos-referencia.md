...
| 2.1–2.5 Captura de orientação | 🟡 Os três modos implementados em `app/bagua-planta/page.tsx`: Modo A (grau decimal, Montanha ao vivo, 3 leituras), Modo B (bússola virtual, ADR 0011) e Modo C (mapa/satélite, ADR 0012). Referência de Norte explícita + conversão magnético↔verdadeiro implementadas (ADR 0014), com a **declinação informada pelo consultor** — o modelo WMM/IGRF em si é deliberadamente não implementado, por falta de fonte primária dos coeficientes. Ainda faltam: cálculo automático da declinação (depende dessa tabela oficial), detecção de Kong Wang (idem, fonte primária), detecção de interferência magnética por magnitude de campo (suporte de navegador insuficiente, ADR 0011) e o questionário de determinação de facing (§2.5) |
