...
| 1.7 Tai Ji / centróide / regra do terço | 🟡 Matemática do centróide geométrico real (`calcularTaiJi`) e do lado "setor ausente" da regra do terço (`coberturaPorCelula`/`setoresAusentes`, com escolha de design declarada) implementadas e testadas em `src/lib/poligono.ts` — ver ADR 0009. Nada conectado à UI ainda (a planta é capturada como retângulo, não polígono arbitrário). Lado "extensão" da regra do terço não implementado (ambiguidade real, ver ADR 0009) |
