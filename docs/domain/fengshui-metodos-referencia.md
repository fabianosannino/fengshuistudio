...
| 2.1–2.5 Captura de orientação | 🟡 Modo A (grau decimal, Montanha ao vivo, assistente de 3 leituras) e Modo B (bússola virtual, ver ADR 0011) implementados em `app/bagua-planta/page.tsx`. Ainda faltam: referência magnético/verdadeiro (declinação WMM/IGRF), detecção de Kong Wang (pendente de fonte primária), detecção de interferência magnética por magnitude de campo (suporte de navegador insuficiente, ver ADR 0011), Modo C (mapa) e o questionário de facing |
