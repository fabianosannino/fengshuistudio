...
> 🟡 **Referência resolvida; modelo geomagnético deliberadamente não.** Desde 2026-07-26 a referência de Norte é dado de primeira classe (`src/lib/declinacao-magnetica.ts`, persistida em `bagua_entrada.orientacao_referencia`, declarada na origem por cada modo: A/B → magnético, C → verdadeiro) e a conversão entre as duas está implementada e testada — ver **ADR 0014**. O problema mais grave era anterior ao modelo: os três modos escreviam no mesmo campo sem dizer qual referência usavam, e nem o sistema sabia depois. **A declinação é informada pelo consultor** (link para a calculadora oficial do NOAA na UI), não calculada: WMM/IGRF exigem uma tabela oficial de ~90 coeficientes de Gauss que não foi obtida de fonte primária, e um coeficiente errado não falha de forma visível — só desloca a Montanha silenciosamente, que é exatamente o dano a evitar. `converterLeitura` é fail-closed (devolve null em vez de assumir declinação zero), e a UI avisa quando as duas referências caem em Montanhas diferentes. Ainda escopo novo: `confidence`/`samples` do modelo `OrientationReading` completo.
...
> ✅ **Implementado (2026-07-26), revertendo a decisão anterior por pedido explícito do usuário.** `src/lib/bussola-dispositivo.ts` (matemática pura de outlier/confiança, MAD com z-score modificado sobre distância circular) + `app/components/BussolaDispositivo.tsx` (calibração, amostragem, rosa dos ventos, bloqueio em confiança baixa) — ver ADR 0011 para a análise completa, incluindo a salvaguarda extra (não presente neste documento) de recusar qualquer leitura quando o navegador só expõe orientação relativa, em vez de apresentá-la como se fosse Norte. **Não implementado, por design:** detecção de interferência por magnitude do campo magnético (Generic Sensor API `Magnetometer` tem suporte de navegador raro demais) e a faixa de Kong Wang no desenho da rosa dos ventos (depende da verificação da seção 1.4, ainda pendente de fonte primária).
...
> ✅ **Implementado (2026-07-26) com Google Maps Platform** — provedor escolhido pelo usuário, já que o modo tem dependência externa paga. `src/lib/orientacao-mapa.ts` (derivação do facing, testada) + `app/components/OverlayAlinhamentoMapa.tsx` (mover/rotacionar/escalar/opacidade, testado isoladamente) + `app/components/MapaAlinhamento.tsx` (Maps JS API, geocodificação, satélite). Ver **ADR 0012** — incluindo a ressalva importante de que a camada que fala com o Google **não pôde ser testada de ponta a ponta** sem uma chave real (só foi verificado que a CSP não a bloqueia e que o caminho sem-chave falha de forma segura); validação num deploy real com chave configurada ainda é necessária. **Fora de escopo, por design:** os "ganhos extras" listados acima (vias em T, curvas, água, prédios vizinhos, sombra) — são geoprocessamento/análise de imagem, não UI de mapa. O facing é entregue como Norte **verdadeiro**, rotulado como tal, sem correção de declinação (mesmo gap do Modo A).
...
> ✅ **Implementado (2026-07-26) — ADR 0016.** `src/lib/facing.ts` + `app/components/QuestionarioFacing.tsx`, painel retrátil no modo Bússola. O consultor descreve as faces candidatas e marca os critérios; o módulo pontua pela hierarquia acima e, **quando o julgamento é ambíguo, devolve as DUAS hipóteses concorrentes** com seus graus e Montanhas, permitindo alternar entre elas — a divergência fica explícita, como esta seção exige. Dispara alerta próprio quando a face vencedora ganhou **apenas** pela porta principal (o erro clássico que esta seção alerta). Fail-closed: sem critério marcado, não elege face nenhuma. Os **pesos numéricos são escolha própria declarada** (esta seção dá a ordem, não os números), com duas propriedades travadas por teste: a porta principal perde de qualquer critério isolado, e nenhum critério isolado vence os dois mais fortes somados. **Não implementado:** renderizar as duas cartas lado a lado — alternar entre hipóteses e recalcular é a aproximação entregue; duas cartas simultâneas exigiriam duplicar todo o pipeline de diagnóstico.
...
> ✅ **Implementado (2026-07-26) — `src/lib/sintese-metodos.ts` + `avaliacao-setor.ts`, ver ADR 0013.** A hierarquia de precedência acima está codificada em `PERFIS_METODOS`, e `resolverConflito` a aplica. As duas linhas que não são só ordem viraram propriedades explícitas: BaZi tem `podeCriarRecomendacao: false` (pode discordar e ser reportado, nunca decide sozinho) e BTB tem `isolado: true` (descartado com aviso se aparecer junto de método de bússola; sozinho decide normalmente). A **invariante de honestidade** é estrutural: `resolverConflito` sempre devolve as divergências com a razão da perda, e não existe caminho no código que as silencie. O conflito canônico "Ba Zhai bom vs. Fei Xing 5 Amarelo" é testado com dados reais (carta do Período 8 + grade anual). O gatilho para implementar agora foi a ressalva da nota abaixo ter deixado de valer — já existem 2+ métodos aditivos gerando conflito de verdade. **Não implementado:** a regra de moradores com necessidades opostas, que depende de um modelo de morador-por-cômodo ausente no schema.
...
> 🟡 **Tipo implementado; migração parcial e deliberadamente limitada (ADR 0015).** `gerarRemedios` (`src/lib/remedios.ts`) produz `Remedio[]` com proveniência completa para as **3 fontes estruturadas** (conflitos cômodo×setor, geometria da regra do terço, estratégia dos Cinco Elementos), alimentando a seção "Plano de Ação" do relatório ordenada por custo/reversibilidade. As **94 dicas em texto livre** de `SETOR_DICAS`/`CRITERIO_DICAS` **não** foram classificadas: atribuir força de evidência a uma recomendação de Feng Shui é julgamento de literatura clássica por afirmação, e rotular errado colocaria selo de autoridade clássica em conselho possivelmente moderno. É curadoria de domínio, pendente de quem tem a formação. `Remedio` existe em `src/lib/sintese-metodos.ts` com **todos os campos de proveniência obrigatórios** — a recomendação acima (nascer com `evidenceStrength`, não migrar depois) foi seguida à risca: são campos não-opcionais, então é impossível criar um remédio sem declarar método de origem, força da evidência, custo e reversibilidade. `ordenarRemedios` implementa "custo zero e reversível primeiro". **`src/lib/recomendacoes.ts` não foi tocado** — é o motor de texto em produção (tela/detalhe/PDF) e reescrevê-lo para `Remedio` é refatoração de porte próprio, não efeito colateral da ADR 0013. O desempate por força de evidência (quando custo e reversibilidade empatam) é escolha própria declarada, não citação deste documento.
...
> **Estado revisado em 18/09/2026:** tipo/área do imóvel, anos de construção/reforma, planta, fachada confirmada com referência/origem, declinação manual ou WMM, e nascimento/parâmetro da fórmula do cliente estão capturados. O cadastro independente de mobiliário permite vários ambientes/objetos no mesmo setor, posição, direção e pessoa de referência opcional (PR #217). As três leituras aceitas, referência e conversão passam a ser preservadas (ADR 0060). Ainda faltam moradores como entidades independentes, pavimentos, entorno estruturado e objetivos/restrições completos. Ver a lista canônica de próximos passos, que distingue implementação de homologação.
...
| 1.7 Tai Ji / centróide / regra do terço | Geometria auxiliar e referência fixa implementadas. Editor único confirma bordas e polígonos conectados de falta/excesso; saldo por setor preserva os percentuais brutos (ADR 0058, PR #217). Áreas, centróide e grade não equivalem à validação integral da regra tradicional por escola. Homologação física e corpus independente permanecem pendentes. |
| 2.1–2.5 Captura de orientação | Parcial: captura manual, sensor experimental, mapa e questionário; fachada exige confirmação, referência de Norte e origem. Declinação automática WMM existe em `declinacao-automatica.ts`/API e há conversão explícita. Três leituras originais, referência/conversão, dispersão e alertas de faixa preservados em ADR 0060. Dispersão não é precisão instrumental. Faltam calibração/condições de campo, proveniência completa de sensor/mapa, Kong Wang validado e piloto. |
| Método 1 — Formas | 🟡 Sha interno em boa parte coberto pelo checklist Fluxo de Chi; Sha externo e classificação por escala não |
| Método 2 — Ba Zhai (Oito Mansões) | Parcial: classificação por assento/octantes e Ming Gua solar explícitos; cadastro persistente de mobiliário em tela separada com ambiente, setor, posição/direção e pessoa de referência opcional (PR #217). Leituras originais em ADR 0060. Restam moradores independentes, política para necessidades conflitantes, Formas, variantes/vetores externos e piloto. Não apresentar como Clássico completo. |
| Método 3 — Estrelas Voadoras | Experimental: mapa simplificado por oito octantes e estrela anual. Não implementa voo completo das 24 Montanhas, substituição, todas as combinações ou camada mensal. Adaptadores/relatório declaram limitações (ADR 0056); cartas externas e variante precisam de revisão antes de promover o método. |
...
| Parte IV — Síntese/conflitos | 🟡 Motor implementado, testado e **conectado ao relatório** (`sintese-metodos.ts` + `avaliacao-setor.ts` + `sintese-imovel.ts`, ADR 0013): hierarquia de precedência dos 7 métodos, resolução de conflito com divergência sempre preservada, BaZi que não decide sozinho, BTB isolado com aviso, e tipo `Remedio` com proveniência obrigatória. A seção **"Onde as escolas divergem neste imóvel"** existe no relatório (`app/consultas/[id]/relatorio/page.tsx`), com a invariante de honestidade da Parte IV cumprida: mostra o método que prevaleceu, o que discordou e a razão da perda. **Sobre «migrar `recomendacoes.ts` para `Remedio`», que esta linha pedia até 12/08:** a ponte já existe e está no relatório — ele chama `gerarRemedios`, e 68 das 76 dicas acionáveis viram `Remedio` com fonte, localizador e citação. Fundir o motor de texto no tipo seria **regressão**, não conclusão: quem decide *quais* dicas se aplicam são os limiares de score e as notas de critério em `recomendacoes.ts`, e replicar isso do outro lado criaria duas fontes divergindo — é o que o cabeçalho de `remedios.ts` recusa explicitamente, e o que o ADR 0015 delimita. As 8 dicas sem enunciado localizável seguem como texto sem selo, por decisão registrada (ADR 0017), não por pendência. **Falta de verdade:** a regra de moradores com necessidades opostas, que depende do modelo morador-por-cômodo ausente no schema |
