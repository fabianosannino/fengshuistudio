...
| 1.7 Tai Ji / centróide / regra do terço | Geometria auxiliar implementada: centróide real (`calcularTaiJi`) e contorno persistido em `bagua_entrada.tai_ji_poligono`. Editor/resumo usam as bordas e divisórias fixas da análise (`contorno-na-grade.ts`, ADR 0057); recuos/extensões não reposicionam o Ba Guá. Ausência ainda é aproximação por área, não validação integral da regra linear. Percentuais/scores continuam nas marcações retangulares; integração semântica e homologação por escola pendentes. Ver também ADR 0009/0010. |
...
| Método 2 — Ba Zhai (Oito Mansões) | ✅ Kua da casa + compatibilidade implementados; **fórmula de Ming Gua corrigida nesta revisão**. Regra 坐凶向吉 (`posicionamento-mobiliario.ts`) implementada e com calculadora interativa em `app/bagua-planta/page.tsx` (Escola da Bússola) — consultor informa data/gênero de quem usa o objeto, localização e direção, avalia na hora. **Ainda não persiste** por cômodo/consulta (calculadora ad-hoc, sem gravação); captura estruturada por cômodo fica para uma iteração futura, quando houver modelo de morador-por-cômodo. Score ponderado por tempo de permanência e política de conflito entre moradores não implementados |
| Método 3 — Estrelas Voadoras | ✅ Carta natal base implementada e testada, com simplificação documentada (par/ímpar em vez de Yuan Long completo); estrela anual implementada (`estrela-anual.ts`). Ti Gua, estruturas da carta, combinações além da Estrela 5, e camada mensal não implementados |
