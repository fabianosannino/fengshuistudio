...
| 2.1–2.5 Captura de orientação | 🟡 Os três modos de captura implementados em `app/bagua-planta/page.tsx`: Modo A (grau decimal, Montanha ao vivo, assistente de 3 leituras), Modo B (bússola virtual, ADR 0011) e Modo C (mapa/satélite, ADR 0012 — camada Google não validada em deploy real ainda). Ainda faltam: conversão magnético↔verdadeiro (declinação WMM/IGRF), detecção de Kong Wang (pendente de fonte primária), detecção de interferência magnética por magnitude de campo (suporte de navegador insuficiente, ADR 0011) e o questionário de determinação de facing (§2.5) |
