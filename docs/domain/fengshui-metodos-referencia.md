...
| Parte/Método | Estado |
|---|---|
| 1.1 Wu Xing (5 movimentos, 3 ciclos) | ✅ Implementado — `src/lib/cinco-elementos.ts` |
| 1.2 Bagua/trigramas, Lo Shu (direção→número) | ✅ Implementado — `oito-mansoes.ts`, `estrelas-voadoras.ts` |
| 1.3 Trajetória de voo Lo Shu | ✅ Implementado e testado contra carta publicada — `estrelas-voadoras.ts` |
| 1.4 24 Montanhas / Kong Wang | 🟡 Não implementado; Kong Wang precisa verificação de fonte primária antes de codificar |
| 1.5 Períodos San Yuan | ✅ Implementado como fórmula cíclica — `estrelas-voadoras.ts` |
| 1.6 Ano solar / Li Chun | 🟡 Implementado com aproximação (4/fev fixo); precisão real pendente |
| 1.7 Tai Ji / centróide / regra do terço | 🟡 App usa bounding box, não centróide; regra do terço não implementada |
| 2.1–2.5 Captura de orientação | 🟡 Só modo manual simples existe; magnético/verdadeiro, 3-leituras, mapa e questionário de facing são escopo novo |
