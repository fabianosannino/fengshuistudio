...
| 2.1–2.5 Captura de orientação | 🟡 Só modo manual simples existe; magnético/verdadeiro, 3-leituras, mapa e questionário de facing são escopo novo. A aritmética circular que isso vai precisar (média/desvio circular) já existe em `src/lib/graus.ts` (shared kernel P0) |
