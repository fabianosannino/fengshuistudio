...
| Parte IV — Síntese/conflitos | 🟡 Motor implementado e testado (`sintese-metodos.ts` + `avaliacao-setor.ts`, ADR 0013): hierarquia de precedência dos 7 métodos, resolução de conflito com divergência sempre preservada, BaZi que não decide sozinho, BTB isolado com aviso, e tipo `Remedio` com proveniência obrigatória. Conflito canônico (Ba Zhai vs. Estrela 5) testado com dados reais. **Falta:** integrar à UI/relatório (o motor não tem call site ainda) e a regra de moradores com necessidades opostas (depende de modelo morador-por-cômodo ausente no schema) |
