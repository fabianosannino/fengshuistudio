...
| 1.7 Tai Ji / centróide / regra do terço | ✅ Regra do terço completa: centróide geométrico real (`calcularTaiJi`), setor ausente (`setoresAusentes`) e extensão (`setoresExtensao`, algoritmo próprio de "maior corpo retangular cheio") implementados e testados em `src/lib/poligono.ts` — ver ADR 0009. 🟡 Nada conectado à UI ainda (a planta é capturada como retângulo, não polígono arbitrário; editor desacoplado existe, ver ADR 0010, mas não integrado à foto real) |
